...
//	usleep(20000);
    ...
    // Decode
    ret      = srsran_ulsch_decode(&q->ul_sch, cfg, q->q, q->g, c, out->data, &out->uci, cfg->beta_factor);
    out->crc = (ret == 0);
    ...
    out->decode_realtime = q->ul_sch.tb_dec_time;

    // Save O_cqi for power control
    cfg->last_O_cqi = srsran_cqi_size(&cfg->uci_cfg.cqi);
    ret             = SRSRAN_SUCCESS;

    cfg->meas_time_value = out->decode_realtime;
                              ...
                              uint32_t               str_len)
{
  ...
  len = srsran_print_check(
      str, str_len, len, ", crc=%s, avg_iter=%.1f, total_iter=%d, no_cbs=%d, stored=%d, cbs_ok=%d, cbs_ko=%d, k1=%d, k2=%d", 
          res->crc ? "OK" : "KO", res->avg_iterations_block, res->total_iterations_block, res->no_cbs, res->no_cbs_stored, res->no_cbs_ok, res->no_cbs_ko, res->cb_K1, res->cb_K2);
