...
int mac::crc_info(uint32_t tti_rx, uint16_t rnti, uint32_t enb_cc_idx, uint32_t nof_bytes, bool crc, bool orig_crc)
{
  logger.set_context(tti_rx);
  srsran::rwlock_read_guard lock(rwlock);
...
int mac::snr_info(uint32_t tti_rx, uint16_t rnti, uint32_t enb_cc_idx, float snr, float snr_custom, ul_channel_t ch)
{
  logger.set_context(tti_rx);
  srsran::rwlock_read_guard lock(rwlock);
  ...
  return scheduler.ul_snr_info(tti_rx, rnti, enb_cc_idx, snr, snr_custom, (uint32_t)ch);
}
