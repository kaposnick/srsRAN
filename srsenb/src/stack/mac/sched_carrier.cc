...
void sched::carrier_sched::carrier_cfg(const sched_cell_params_t& cell_params_)
{
  ...
  } else if (cell_params_.sched_cfg->sched_policy == "time_sched_ai") {
	sched_algo.reset(new sched_ai{*cc_cfg, *cell_params_.sched_cfg});
	logger.info("Using the AI scheduler for cc=%d", cc_cfg->enb_cc_idx);
  }
  else {
