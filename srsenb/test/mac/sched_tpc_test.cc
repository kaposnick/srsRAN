...
int test_finite_target_snr()
{
  ...
  for (uint32_t i = 0; i < 100 and tpcfsm.get_ul_snr_estim(0) < 14; ++i) {
    tpcfsm.set_snr(15,0,0,0);
    tpcfsm.set_snr(15,0,0,1);
    tpcfsm.new_tti();
  }
  ...
  int snr_diff = 10;
  tpcfsm.set_snr(target_snr + snr_diff, 0, 0, tpc::PUSCH_CODE);
  tpcfsm.set_snr(target_snr + snr_diff, 0, 0, tpc::PUCCH_CODE);
...
int test_undefined_target_snr()
{
  ...
  int snr_info = 10;
  tpcfsm.set_snr(snr_info, 0,0, tpc::PUSCH_CODE);
  tpcfsm.set_snr(snr_info, 0, 0,tpc::PUCCH_CODE);
  sum_pusch = 0;
  sum_pucch = 0;
  for (uint32_t i = 0; i < 100; ++i) {
    tpcfsm.new_tti();
