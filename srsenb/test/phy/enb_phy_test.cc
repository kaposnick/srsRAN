...
public:
  ...
  int snr_info(uint32_t tti, uint16_t rnti, uint32_t cc_idx, float snr_db, float snr_db_custom, ul_channel_t ch) override
  {
    notify_snr_info();
    return 0;
  }
  ...
  int crc_info(uint32_t tti, uint16_t rnti, uint32_t cc_idx, uint32_t nof_bytes, bool crc_res, bool orig_crc) override
  {
    std::lock_guard<std::mutex> lock(phy_mac_mutex);
