...
public:
  ...
  int ul_crc_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, bool crc, bool orig_crc) final;
  ...
  int ul_snr_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, float snr, float snr_custom, uint32_t ul_ch_code) final;
  ...
  void                                 update_beta_factor(uint32_t update_beta_factor, uint16_t gain);
