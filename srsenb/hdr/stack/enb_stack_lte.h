...
public:
  ...
  int snr_info(uint32_t tti_rx, uint16_t rnti, uint32_t cc_idx, float snr_db, float snr_db_custom, ul_channel_t ch) final
  {
    return mac.snr_info(tti_rx, rnti, cc_idx, snr_db, snr_db_custom, ch);
  }
  ...
  int crc_info(uint32_t tti, uint16_t rnti, uint32_t enb_cc_idx, uint32_t nof_bytes, bool crc_res, bool orig_crc) final
  {
    return mac.crc_info(tti, rnti, enb_cc_idx, nof_bytes, crc_res, orig_crc);
  }
