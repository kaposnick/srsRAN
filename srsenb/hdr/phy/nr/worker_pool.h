...
private:
  ...
  public:
    ...
    int snr_info(uint32_t tti, uint16_t rnti, uint32_t cc_idx, float snr_db, float snr_custom_db, ul_channel_t ch) override { return 0; }
    ...
    int crc_info(uint32_t tti, uint16_t rnti, uint32_t cc_idx, uint32_t nof_bytes, bool crc_res, bool orig_crc) override { return 0; }
